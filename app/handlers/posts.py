...
async def _(message):
    await bot.edit_message_reply_markup(
        settings.channelID, message.message_id, reply_markup=create_post_kb()
    )
    await insert_post(message.message_id)
    await bot.pin_chat_message(settings.channelID, message.message_id)
    await bot.delete_message(settings.channelID, message.message_id+1)
